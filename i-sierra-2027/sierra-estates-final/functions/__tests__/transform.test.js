...
describe('normalizeProperty', () => {
  it('maps a complete raw payload to the canonical shape', () => {
    expect(normalizeProperty({ title:'Sea View Villa', price:'4,500,000', location:'New Cairo', source:'PropertyFinder' })).toEqual({
      title:'Sea View Villa', price:4500000, location:'New Cairo', source:'PropertyFinder', isAvailable:true,
    });
  });
  it('applies safe defaults when fields are missing', () => {
    expect(normalizeProperty({})).toEqual({ title:'Untitled Property', price:0, location:'Unknown', source:'Scraper Bot', isAvailable:true });
  });
