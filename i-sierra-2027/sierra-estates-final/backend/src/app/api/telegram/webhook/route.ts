...
export async function POST(req: Request) {
  try {
    ...
    if (text === '/start') {
      await sendMessage(`
<b>Welcome to Sierra Estates Bot</b>
...
---
      ...
      try {
        ...
        // Conversation / Inquiry Mode for Investment Stakeholders!
        ...
        const queryText = text.startsWith('/ag') ? text.replace('/ag', '').trim() : text;

        const result = await OmnichannelChatService.handleIncomingMessage({
            platform: 'telegram',
            senderId: String(chatId),
            senderName: message.from?.first_name || 'Telegram Stakeholder',
            text: queryText || "Hello! I am Antigravity. How can I assist your operations today?"
        });

        await sendMessage(result.replyText);
    }
