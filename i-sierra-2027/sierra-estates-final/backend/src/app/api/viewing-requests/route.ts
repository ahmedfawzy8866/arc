...
// Plain substring checks instead of a backtracking-prone regex (avoids ReDoS on attacker-controlled input).
function isValidEmail(email: unknown): boolean {
  if (typeof email !== 'string' || email.length === 0 || email.length > 254 || /\s/.test(email)) {
    return false;
  }
