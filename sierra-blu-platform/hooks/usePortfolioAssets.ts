...
import type { PortfolioAsset } from "@/lib/models/schema";

interface PFSyncResult {
  success: boolean;
  syncedAt: string;
}

const pushAssetToPF = async (asset: PortfolioAsset): Promise<PFSyncResult> => {
  return { success: true, syncedAt: new Date().toISOString() };
};

const getPFAssetAnalytics = async (assetId: string) => {
  return { views: 0, interests: 0, reveals: 0 };
};

export interface AssetWithAnalytics extends PortfolioAsset {
  pfViews?: number;
...
export function usePortfolioAssets(options: {
  ...
  const syncAsset = useCallback(
    async (asset: PortfolioAsset): Promise<PFSyncResult> => {
