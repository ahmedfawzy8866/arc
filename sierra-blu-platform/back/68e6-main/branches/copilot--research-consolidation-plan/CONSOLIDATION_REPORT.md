...
## 7. 🧠 Ideas Backlog (Documented, Not Implemented)

The following items are kept as **documented ideas only** and are not merged into runtime code yet:

- **MaintenanceMonitor service** (`imports/New-folder/docs/implementation_plan.md`)
- **ImageLinkHub workflow enhancements** (`imports/New-folder/docs/implementation_plan.md`)
- **OpenClaw classification improvements** (`imports/New-folder/docs/implementation_plan.md`)
- **GoogleIntegrationService (Sheets/Calendar expansion)** (`imports/New-folder/docs/implementation_plan.md`)
- **Frontend prototype aesthetics and token experiments** (`imports/Sierra-Blu-Systm/README.md`)

These remain pending explicit implementation tasks, tests, and product review.

---

## 8. 🛡️ Merge Safety Rules (Canonical Repo First)

- Destination repository (`sierra-blu-platform`) is the **single canonical deployable codebase**.
- Related repositories are treated as sources for **selective import only**.
- Unrelated, inaccessible, third-party, or fork-risk repositories must be excluded from direct code merge until legal/technical review is complete.
- Source repositories should only be deleted after checklist sign-off, validation in this repo, and human approval.
