...
const Map3D = dynamic(() => import('@/components/UI/Map3D'), {
  ssr: false,
  loading: () => <div className="w-full h-[600px] bg-[#0A1520] animate-pulse flex items-center justify-center text-[#E9C176] font-serif border border-[rgba(233,193,118,0.18)] rounded-xl">Initializing 3D Intelligence Map...</div>,
});
...
const COPY = {
  en: {
    ...
    heroItalic: '',
    heroSub: 'Sierra-Blu Realty | Beyond Brokerage',
    heroDesc: 'Unlock the market\'s best opportunities with AI-powered real estate intelligence.',
...
export default function LandingPage() {
  ...
  // TREF Style Hero Slider State
  const [heroIdx, setHeroIdx] = useState(0);
  const HERO_SLIDES = [
    'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=2000&q=80',
    'https://images.unsplash.com/photo-1613490900233-08ba19cc245c?w=2000&q=80',
  ...
  return (
    <div style={{ minHeight: '100vh', background: th.bg, color: th.text, transition: 'background .5s, color .5s' }} dir={T.dir}>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: '#000' }}>
        ...
        {HERO_SLIDES.map((src, i) => (
          <div 
            ...
            style={{ 
              ...
              opacity: i === heroIdx ? (mode === 'dark' ? 0.6 : 0.8) : 0, 
        ...
        <div style={{ position: 'absolute', inset: 0, background: mode === 'dark' ? 'radial-gradient(circle at center, rgba(10,21,32,0.2) 0%, rgba(10,21,32,0.8) 100%)' : 'radial-gradient(circle at center, rgba(0,0,0,0.1) 0%, rgba(0,0,0,0.5) 100%)', zIndex: 1 }} />
        
        <ParticleCanvas />

        <div style={{ ...sec, position: 'relative', zIndex: 2, width: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, animation: loaded ? 'fadeUp .6s ease .1s both' : 'none' }}>
            <div style={{ width: 28, height: 1, background: G }} />
            <span style={{ fontSize: 11, letterSpacing: isAr ? '.04em' : '.28em', color: G, fontWeight: 600, fontFamily: isAr ? "'Cairo', sans-serif" : "'Jost', sans-serif", textTransform: 'uppercase' }}>
              {T.brand} {T.sub}
            </span>
            <div style={{ width: 28, height: 1, background: G }} />
          </div>

          <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(48px, 7vw, 100px)', fontWeight: 300, color: '#fff', lineHeight: 1.05, letterSpacing: '-0.02em', margin: '24px 0 20px', textShadow: '0 4px 24px rgba(0,0,0,0.4)', animation: loaded ? 'fadeUp .8s ease .2s both' : 'none' }}>
            {T.heroH1[0]}<br />{T.heroH1[1]}
            {T.heroItalic && <><br /><em className="gold-text" style={{ fontStyle: 'italic' }}>{T.heroItalic}</em></>}
          </h1>

          <div style={{ fontSize: isAr ? 16 : 14, letterSpacing: isAr ? '.04em' : '.22em', textTransform: 'uppercase', color: G, fontWeight: 500, fontFamily: isAr ? "'Cairo', sans-serif" : "'Jost', sans-serif", marginBottom: 40, animation: loaded ? 'fadeUp .8s ease .3s both' : 'none' }}>
            {T.heroDesc}
          </div>

          <Link href="/investments" style={{ textDecoration: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#E9C176', color: '#000', border: 'none', cursor: 'pointer', fontFamily: isAr ? "'Cairo', sans-serif" : "'Jost', sans-serif", fontSize: 13, fontWeight: 600, letterSpacing: isAr ? '.02em' : '.15em', textTransform: 'uppercase', padding: '16px 42px', borderRadius: 4, boxShadow: '0 8px 30px rgba(233,193,118,0.3)', animation: loaded ? 'fadeUp .8s ease .4s both' : 'none', transition: 'all 0.3s ease' }} className="hover:scale-105 hover:bg-white">
            {T.btnDiscover}
          </Link>

        </div>

        <div style={{ position: 'absolute', bottom: 40, left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, opacity: 0.6, zIndex: 2 }}>
          <span style={{ fontSize: 9, letterSpacing: '.24em', textTransform: 'uppercase', color: '#fff', fontFamily: "'Jost', sans-serif" }}>{T.scroll}</span>
          <div className="animate-shimmer" style={{ width: 1, height: 40, background: `linear-gradient(180deg,${G},transparent)` }} />
        </div>
      </section>
