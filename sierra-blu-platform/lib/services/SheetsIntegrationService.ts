...
const validateEnvVars = () => {
  ...
  if (!spreadsheetId || !apiKey) {
    console.warn('[SheetsIntegration] Google Sheets API not configured. Read-only CSV fallback will be used.');
    return { spreadsheetId: null, apiKey: null };
  }
...
export class SheetsIntegrationService {
  ...
  /**
   * Appends a row to a specific Google Sheets tab.
   * Requires GOOGLE_SHEETS_SPREADSHEET_ID and GOOGLE_SHEETS_API_KEY env vars.
   */
  static async appendRow(
    sheetName: string,
    rowData: Record<string, string | number | boolean>
  ): Promise<boolean> {
    const { spreadsheetId, apiKey } = validateEnvVars();

    if (!spreadsheetId || !apiKey) {
      console.warn(
        '[SheetsIntegration] Cannot append to Google Sheets: missing configuration. ' +
        'Set GOOGLE_SHEETS_SPREADSHEET_ID and GOOGLE_SHEETS_API_KEY to enable.'
      );
      return false;
    }
    ...
    try {
      ...
      const response = await fetch(
        `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(range)}:append?key=${apiKey}`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        ...
        console.error('[SheetsIntegration] API error:', error);
        return false;
      }

      console.log(`[SheetsIntegration] Row appended to "${sheetName}" successfully`);
      return true;
    } catch (error: any) {
      console.error('[SheetsIntegration] Failed to append row:', error.message);
      return false;
    }
  }
}
