...
try {
  if (!admin.apps.length) {
    ...
    if (serviceAccount) {
      ...
      // Support both raw JSON and base64-encoded JSON (base64 starts with 'ey' = '{' encoded)
  ...
  console.error(
    '[firebase-admin] CRITICAL: Running without database access. All write operations will silently fail. ' +
    'Set FIREBASE_SERVICE_ACCOUNT_JSON or (FIREBASE_CLIENT_EMAIL + FIREBASE_PRIVATE_KEY + FIREBASE_PROJECT_ID) in your environment.'
  );
}
