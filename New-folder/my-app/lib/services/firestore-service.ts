...
export const Leads = {
  create: (data: Parameters<typeof createDocument>[1]) => createDocument(COLLECTIONS.leads, data),
  get: (id: string) => getDocument(COLLECTIONS.leads, id),
  update: (id: string, data: Parameters<typeof updateDocument>[2]) => updateDocument(COLLECTIONS.leads, id, data),
  remove: (id: string) => deleteDocument(COLLECTIONS.leads, id),
  query: (opts?: QueryOptions) => queryDocuments(COLLECTIONS.leads, opts),
  subscribe: (cb: Parameters<typeof subscribeToCollection>[1], opts?: Parameters<typeof subscribeToCollection>[2]) =>
    subscribeToCollection(COLLECTIONS.leads, cb, opts),
};
